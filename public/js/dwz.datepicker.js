    ...
    $.fn.datepicker = function (opts) {
        ...
        return this.each(function () {
            ...
            function generateCalendar(dp) {
                ...
                if (!dp.hasDay()) {
                    $(setting.dayNames$).html('');
                    var monthlist = '';
                    for (ml = 1; ml <= 12; ml++) {
                        var _date = new Date(dw.year, ml-1);
                        var _ctrClass = (_date >= minDate && _date <= maxDate) ? '' : 'other disabled';
                        if (ml > 9) {
                            monthlist += '<dd class="'+_ctrClass+'" chMonth="' + (ml - dw.month) + '" day="1" style="width:88px;float:left">' + dw.year + '-' + ml + '</dd>';
                        } else {
                            monthlist += '<dd class="'+_ctrClass+'" chMonth="' + (ml - dw.month) + '" day="1" style="width:88px;float:left">' + dw.year + '-0' + ml + '</dd>';
                        }
                    }
                    var $days = $(setting.days$).html(monthlist).find("dd");
